...
- **MMM "Run Now" Spawn Fix + Status Badges**: POST `/api/mmm/run` no longer spawns the intraday pipeline with `detached: true` — Replit reaps orphaned process groups, so detached runs were silently killed every time (zombie "running" rows). The child now stays attached to the server process (stdio inherited to workflow logs) and completes reliably (~60-90s). The stale-run guard also relabels dead "running" rows to failed so the history table never shows a perpetual spinner. `RunStatusBadge` now renders red "Failed" for failed/error statuses (previously anything non-success/error showed as "Running"). **Autoscale caveat**: background work after a 202 response is only reliable in the dev workspace or on a Reserved VM deployment — if the dashboard is ever published on Autoscale, the Run Now child may be killed when CPU throttles post-response; the Scheduled Deployments handle real automation regardless.
- **IMS Scraper Credentials Fixed**: IMS portal login (`admin.tmginteractive.com`) now uses dedicated `IMS_USERNAME`/`IMS_PASSWORD` env vars (portal login from Mike's credentials file). The old `TMG_REVENUE_USER`/`TMG_REVENUE_PASSWORD` secrets are Basic-auth API credentials for `services.tmginteractive.com` (used by an existing Express route) and are NOT valid portal logins — the config fallback to them was why IMS login always failed. IMS now pulls creative-level revenue rows (aff_id/sub_id compound keys) successfully; scraper verifies login success, detects `syslogin.aspx?ex=Y` failures, and logs out explicitly (portal allows one active session per account).

## Previous Updates (July 4, 2026)
- **MMM Ad Revenue Intelligence Pipeline**: Integrated a Python data pipeline that pulls ad spend (Meta, creative-level Google Ads) and revenue (IMS/TMG, AfterOffers, Interactive Offers, Zenect, Google Sheet) and joins them on a `compound_key` (affId-subId) with impression-based fallback allocation. Ported from SQLite to the existing Postgres. New tables: `mmm_ad_spend`, `mmm_ims_revenue`, `mmm_ao_revenue`, `mmm_daily_spend`, `mmm_source_daily`, `mmm_performance_daily` (dashboard snapshot), `mmm_run_log` (run lifecycle). Package layout: `mmm-pipeline-package/{scrapers,pipeline,config,debug}`. Credentials come from Replit Secrets/env via `config/config.py` (no `credentials.json`); every scraper skips gracefully with 0 rows when its credentials are absent, so the dashboard renders empty until keys are provided. All dates use America/New_York (EST). Playwright chromium runs on Replit via the `playwright-driver` nix package. Runners: `pipeline/run_daily.py` (yesterday, full incl. Google Sheet + optional Claude summary) and `pipeline/run_intraday.py` (today, no sheet). Admin dashboard at `/admin/mmm` (sidebar: "Ad Revenue (MMM)"): 30-day performance table with ROAS color-coding, creative detail time-series dialog, and a Pipeline Status tab with run history + a "Run Now" button (POST `/api/mmm/run` spawns the intraday runner detached, with a stale-run guard). Read APIs: GET `/api/mmm/performance`, `/api/mmm/creative/:key`, `/api/mmm/runs`. Two Scheduled Deployments (created in the Deployments UI) drive automation: daily `python3 mmm-pipeline-package/pipeline/run_daily.py` at 14:00 UTC (9am EST, cron `0 14 * * *`) and intraday `python3 mmm-pipeline-package/pipeline/run_intraday.py` every 2 hours (cron `0 */2 * * *`).
